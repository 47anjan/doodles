...
import { GroceryList } from "@/app/recipes/_components/GroceryList";
      ...
      <div className="space-y-8">
        <header className="space-y-4 pb-6 border-b border-gray-100">
          <div className="flex justify-between items-start gap-4">
            ...
            {user && <FavoriteStatus {...saveRecipe} />}
          </div>
          <div className="flex flex-wrap gap-2">
            {recipe.cuisines?.map((item, i) => (
              <Link
                key={i}
                className={`${badgeVariants()}  transition-colors`}
                href={`/search?query=${item.toLocaleLowerCase()}&type=recipes`}
              >
                <GiKnifeFork className="mr-1.5 h-4 w-4" />
                {item}
              </Link>
            ))}
          </div>
          ...
          <div className="space-y-8">
            <div className="grid sm:grid-cols-3 gap-4">
              ...
              <div className="p-4 bg-white rounded-xl border border-gray-100 ">
                ...
                <p className="text-2xl font-medium">{recipe.readyInMinutes}m</p>
              </div>
              ...
              <div className="p-4 bg-white rounded-xl border border-gray-100 ">
                ...
                <p className="text-2xl font-medium">{recipe.aggregateLikes}</p>
              </div>
            </div>
              ...
              (instruction, index) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-8 h-8 rounded-full bg-orange text-white font-bold flex items-center justify-center">
                      {instruction.number}
                    </div>
